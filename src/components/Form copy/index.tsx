...
function Form() {
  ...
  useEffect(() => {
    const socket = socketIO(VITE_MESSENGER_SERVER_SOCKET_URL!, { transports: ['websocket'] });
    socket.on("connect", () => {
      console.log(socket.id);
    });
    socket.on('ping', (value: any) => {
  ...
  const { data: qrCode, isFetching: isFetchingGenerateQRCode, isLoading: isLoadingGenerateQRCode, isSuccess, refetch: refetchGenerateQRCode } = useGenerateQRCode(data);
  const { data: connected, isFetching: isFetchingAuthentication, isLoading: isLoadingCheckAuthentication, refetch: refetchCheckAuthentication } = useCheckAuthentication(checkAuthentication);
  ...
  const handleCheckFile = async (event: any) => {
    ...
    queryClient.invalidateQueries(['generateQRCOde'])
    queryClient.invalidateQueries(['checkAuthentication'])
  ...
  const handleCheckConnectionWhatsapp = async (event: any) => {

    setLoadingHandleCheckConnectionWhatsapp.on()
    ...
    return new Promise<void>(async (resolve) => {
      setTimeout(() => {
        refetchCheckAuthentication();
        // console.log('REFETCHING...')
        setLoadingHandleCheckConnectionWhatsapp.off()
        resolve();
      }, 2000);
    });
  }
                ...
                <>
                  <Stack spacing={4} textAlign={'left'} gap={6}>
                    <>
                      ...
                      {isFetchingGenerateQRCode || isLoadingGenerateQRCode ? (
                        <Box
                  ...
                  <Stack spacing={4} textAlign={'left'}>
                    <>
                      ...
                      status={successMessage?.status || 'info'}
                      icon={''}
                    /> */}
                      ...
                      {/* {qrCode && qrCode.qrCode && (
                        <> */}
                          ...
                          <Button
                            type={'submit'}
                            colorScheme={'whatsapp'}
                            onClick={handleCheckConnectionWhatsapp}
                            disabled={data && qrCode ? false : true}
                            isLoading={loadingHandleCheckConnectionWhatsapp}
                            loadingText={'Verificando...'}
                            opacity={loadingHandleCheckConnectionWhatsapp ? 0.5 : 1}
                          >
                            Check Whatsapp connection
                          </Button>

                          {/* {connected && checkAuthentication && connected.authenticated ? ( */}
                          {/* {connected && connected.authenticated ? (
                            <>
                              ...
                              <Button
                                ...
                                disabled={data && qrCode && connected.authenticated ? false : true}
                            ...
                            <Text fontSize={'md'}>
                              {checkAuthentication && !loadingHandleSubmitData && !loadingHandleCheckConnectionWhatsapp ? 'Status: Disconnected' : ''}
                            </Text> */}
                          {/* )} */}
                        {/* </>
                      )}  */}
